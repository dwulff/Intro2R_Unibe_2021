...
In this practical you'll practics to ...

1. Run simple and multiple regression analysis in R.
2. How to interpret the results of a regression model.
...
4. Use the `read_csv()` function to read `wein.csv` into the object `wein`.
...
7. Use `names(XX)`, `summary(XX)`, and `View(XX)` to get an overview of the dataset.

8. Run the code below to ensure that all `character` variables are actucally factors. This is helping the models below to interpret catergorical variables.

```{r, echo = TRUE}
# convert everything into a factor
