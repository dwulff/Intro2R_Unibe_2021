...
trial <- read_csv("https://raw.githubusercontent.com/dwulff/Intro2R_Unibe_2021/main/_sessions/Statistics/1_Data/trial_ACTG175.csv")
...
4. Select and run the appropriate test for the appropriate research question.
...
2. Open a new R script. At the top of the script, using comments, write your name and the date. Save it as a new file called `Statistics_practical.R` in the `2_Code` folder.  
...
age_cd4_baseline_test <- cor.test(formula = ~ age + cd40,
                          data = trial)
```
...
```{r}
# GLM predicting a patient's CD4 T cell count at baseline form their age at baseline 
age_cd4_baseline_glm <- glm(formula = cd40 ~ age,
                    data = trial)
...
```{r, eval= FALSE, echo = TRUE}
ggplot(data = trial, mapping = aes(x = symptom, y = cd40, col = symptom)) + 
  stat_summary(geom = "point", fun = "mean", fill = "white", col = "black") +
...
symptomatic_cd4_baseline_test <- t.test(formula = cd40 ~ symptom,
                           data = trial)
```
...
```{r}
# GLM predicting a patient's CD4 T cell count at baseline form their age at baseline 
symptomatic_cd4_baseline_glm <- glm(formula = cd40 ~ symptom,
                        data = trial)
...
1. Using the code below, create a plot showing the distribution of the number of days until a major negative event (`days`) for each treatment arm. Based on this plot, do you expect to find that there were differences in treatment molecules?

```{r, eval= FALSE, echo = TRUE}
ggplot(data = trial, mapping = aes(x = arms, y = days, col = arms)) + 
  stat_summary(geom = "point", fun = "mean", fill = "white", col = "black") +
...
days_molecule_aov <- aov(formula = days ~ arms,
                      data = trial)
```
...
6. Now, print your `days_molecule_tidy` object, how do the results look?
...
7. In order to compare pair-wise differences between groups after conducting an ANOVA, you need to conduct post-hoc tests. Using the `TukeyHSD()` function, conduct pair-wise tests to see which pairs of treatment molecule differ in `days`. Save the result to an object called `days_molecule_aov_post`
`
```{r, eval = FALSE, echo = TRUE}
...
8. Print your `days_molecule_aov_post` object, what do you see?

9. Print 'tidy' results from your `days_molecule_aov_post` object using the `tidy()` function
...
10. Based on what you've seen, what are your conclusions? Which treatment molecule seem to differ?

##### ANOVA as Linear Model

11. Conduct the same analysis you did before, but this time use the `glm()` function. Use the code skeleton below to help you. Do you get the same results as you did using `aov()`? Which outputs are the same and which are different?
...
```{r}
# GLM predicting number of days until a major negative event (days) as a function of treatment arm (molecule)
days_molecule_glm <- glm(formula = days ~ arms,
                     data = trial)
...
days_glm <- glm(formula = days ~ arms + gender + age + wtkg + drugs + race + cd40 + cd80,
                data = trial)
```
...
|File | Rows | Columns |
|:----|:-----|:------|
|[trial_ACTG175.csv](https://raw.githubusercontent.com/dwulff/Intro2R_Unibe_2021/main/_sessions/Statistics/1_Data/trial_ACTG175.csv) | 2139 | 27 |
